...
namespace Drupal\standard_reports\Service;
 ...
 *
 * @package Drupal\standard_reports\Service
 */
class ReportService {
  ...
  public function getSalesSummary(array $dateRange, string $groupBy): array {
    ...
    if (!in_array($groupBy, ['day', 'month', 'year', 'week'], TRUE)) {
      $groupBy = 'day';
    }
    ...
    if ($groupBy !== 'week') {
      ...
      if (method_exists($order_storage, 'getTableMapping')) {
        $table_mapping = $order_storage->getTableMapping();
        $order_table = $table_mapping->getBaseTable();
      }

      $date_expression = $this->buildDateGroupExpression('o.created', $groupBy);

      $query = $this->database->select($order_table, 'o');
      $query->condition('o.created', $start_timestamp, '>=');
      $query->condition('o.created', $end_timestamp, '<=');
      $query->condition('o.state', 'completed');
      $query->addExpression($date_expression, 'period');
      $query->addExpression('COUNT(o.order_id)', 'order_count');
      $query->addExpression('SUM(o.total_price__number)', 'revenue');
      $query->addField('o', 'total_price__currency_code', 'currency');
      ...
      foreach ($results as $row) {
        ...
        $grouped_data[$period]['Orders'] += (int) $row->order_count;
        $grouped_data[$period]['Revenue'] += (float) $row->revenue;
      }
    ...
    foreach ($orders as $order) {
      ...
      $key = $this->getGroupingKey($timestamp, $groupBy);
      ...
      $grouped_data[$key]['Revenue'] += (float) $order->getTotalPrice()->getNumber();
    }
  ...
  /**
   * Builds a database-specific grouping expression for a Unix timestamp field.
   *
   * @param string $field
   *   The SQL field name containing a Unix timestamp.
   * @param string $groupBy
   *   The grouping factor ('day', 'month', 'year').
   *
   * @return string
   *   SQL expression used to group by the requested period.
   */
  protected function buildDateGroupExpression(string $field, string $groupBy): string {
    ...
    switch ($database_type) {
      case 'pgsql':
        $pg_format = strtr($format, ['Y' => 'YYYY', 'm' => 'MM', 'd' => 'DD']);
        return "TO_CHAR(TO_TIMESTAMP($field), '$pg_format')";

      case 'sqlite':
        return "strftime('$sqlite_format', $field, 'unixepoch')";

      case 'mysql':
      case 'mariadb':
      default:
        return "DATE_FORMAT(FROM_UNIXTIME($field), '$mysql_format')";
    }
  }
